...
export function ChatHeader({
  ...
  const isOpenClaw = backendMode === "openclaw";
  const canSwitch = isOpenClaw && !!onSessionPillClick;
  const displayName = isOpenClaw && sessionName ? sessionName : "GoBot";
  ...
  return (
    <>
      <header className="absolute top-0 left-0 right-0 z-10 flex items-center gap-3 border-b border-border/50 px-4 py-3 font-[family-name:var(--font-geist-sans)] backdrop-blur-sm md:px-6" style={{ background: "oklch(from var(--card) l c h / 0.7)" }}>
        ...
        >
          <img src="/logo.png" alt="GoBot" className="h-7 mix-blend-multiply dark:mix-blend-screen dark:invert" />
        </button>
      ...
      {connectionError && (
        <div className="absolute top-[52px] left-0 right-0 z-9 flex items-center justify-center gap-2 bg-destructive/10 px-4 py-1.5 text-xs text-destructive border-b border-destructive/20">
